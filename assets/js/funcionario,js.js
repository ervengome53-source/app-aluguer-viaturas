...
class FuncionarioAlugueis {
            ...
            if (select) {
                select.innerHTML = '<option value="">Selecionar viatura</option>' +
                    resultado.dados.map(v => `
                        <option value="${v.id}" data-preco="${v.preco_dia}">${v.marca} ${v.modelo} - € ${v.preco_dia}/dia</option>
                    `).join('');
            }
        }
    }
    ...
    static calcularPreco() {
        ...
        document.getElementById('total_exibir').innerHTML = `€ ${total.toFixed(2)}`;
    }
                ...
                select.addEventListener('change', () => {
                    ...
                    if (dataFim) {
                        ...
                        if (diasAtraso > 0) {
                            ...
                            document.getElementById('multa_valor').innerHTML = `MZN ${multa.toFixed(2)} (${diasAtraso} dias de atraso)`;
                        } else {
